...
class TimeCode {
 public:
  TimeCode(unsigned int hr = 0, unsigned int min = 0, long long unsigned int sec = 0) {if (hr<0||min<0||sec<0) cout << "Negative Numbers Not Supported" << endl; else t = ((hr*3600)+(min*60)+(sec));}
  TimeCode(const TimeCode& tc){t = tc.t;}
